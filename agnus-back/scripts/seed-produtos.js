...
const Categorias = require(path.join(distRoot, "models", "Categorias")).default;
const sequelize = require(path.join(distRoot, "config", "database")).default;
const ProdutoMutationService = require(path.join(distRoot, "services", "produtoMutation.service")).default;

const QUANTIDADE = Number(process.argv[2] || 20);

const IMAGENS_DIR = path.join(__dirname, "seed-images");
const EXTENSOES_VALIDAS = new Set([".jpg", ".jpeg", ".png", ".webp", ".gif", ".bmp"]);

function listarImagensLocais() {
  ...
  return fs
    ...
    .filter((file) => EXTENSOES_VALIDAS.has(path.extname(file).toLowerCase()))
    .map((file) => path.join(IMAGENS_DIR, file));
}
...
async function seedProdutos() {
  ...
  const imagensLocais = listarImagensLocais();
  if (imagensLocais.length) {
    console.log(`Usando ${imagensLocais.length} imagem(ns) local(is) de ${IMAGENS_DIR}`);
  } else {
    console.log(`Nenhuma imagem em ${IMAGENS_DIR}, usando placeholders do picsum.photos`);
  }
  ...
  for (let i = 0; i < QUANTIDADE; i += 1) {
    ...
    const cores = imagensLocais.length
      ? [
          {
            nome: cor.nome,
            ...
            fotos_upload: [
              {
                buffer: fs.readFileSync(pick(imagensLocais, i)),
                extensao: path.extname(pick(imagensLocais, i)).slice(1),
              },
            ],
          },
        ]
